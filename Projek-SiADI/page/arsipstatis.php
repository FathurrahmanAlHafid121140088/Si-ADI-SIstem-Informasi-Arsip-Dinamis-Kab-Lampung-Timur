...
// Hitung total data dengan status 'Statis'
$count_query = "SELECT COUNT(*) AS total FROM data_dinamis WHERE status = 'Statis'"; 
$count_result = $conn->query($count_query);

if ($count_result->num_rows > 0) {
    $count_row = $count_result->fetch_assoc();
    $total_statis = $count_row["total"];
} else {
    $total_musnah = 0;
}
...
.footer-nama:hover {
  ...
  <nav class="navbar d-flex justify-content-start sticky-top">
    ...
    <div class="table">
        <h2 class="h2">Data Arsip Statis</h2>
        <div class='table-container'>
        <p style="font-weight: 600; font-size: large">Data yang masuk: <?php echo $total_statis ?></p>
...
$sql = "SELECT COUNT(*) FROM data_dinamis WHERE status = 'Statis'";
...
$sql = "SELECT *
        FROM data_dinamis WHERE status = 'Statis' ORDER BY tanggal_masuk DESC
        LIMIT $start, $limit";
$result = $conn->query($sql);
...
function printTable() {
  ...
  printWindow.document.write("<html><head><title>Data Arsip Statis</title>");
  ...
  // Add header h2 here
  printWindow.document.write("<h2 class='h2'>Data Arsip Statis</h2>");
