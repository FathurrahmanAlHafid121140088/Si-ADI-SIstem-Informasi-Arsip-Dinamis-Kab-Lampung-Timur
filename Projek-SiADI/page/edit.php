...
// Variabel untuk menampung data arsip
$arsip_data = [
    'tahun_arsip' => '',
    'skpd' => '',
    'pokok_masalah' => '',
...
// Cek apakah terdapat no_surat_arsip di query string
if (isset($_GET['no_surat_arsip'])) {
    $no_surat_arsip = $_GET['no_surat_arsip'];

    // Ambil data arsip berdasarkan no_surat_arsip
    $sql = "SELECT * FROM data_dinamis WHERE no_surat_arsip = ?";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $uraian_arsip = $_POST['uraian-arsip'];
    ...
    // Periksa apakah data sudah ada berdasarkan no_surat_arsip
    $check_sql = "SELECT COUNT(*) AS count FROM data_dinamis WHERE no_surat_arsip = ?";
    $check_stmt = $conn->prepare($check_sql);
    $check_stmt->bind_param("s", $no_surat_arsip);
    $check_stmt->execute();
    ...
    if ($check_row['count'] > 0) {
        ...
        $sql = "UPDATE data_dinamis SET tahun_arsip = ?, skpd = ?, pokok_masalah = ?, kode_klasifikasi = ?, uraian_arsip = ?, no_urut_berkas = ?, no_box = ?, no_rak = ?, bentuk_penataan = ?, nama_pemberkas = ?, indeks_judul_arsip = ?, tingkat_pengembangan_arsip = ?, nilai_guna_arsip = ?, kondisi_arsip = ?, status = ?, pemerian_berkas = ?, jumlah_berkas = ?, keterangan = ?, retensi_aktif = ?, retensi_inaktif = ?, tanggal_masuk = ?, lampirkan_file = ? WHERE no_surat_arsip = ?";
        
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("sssssssssssssssssssssss", $tahun_arsip, $skpd, $pokok_masalah, $kode_klasifikasi, $uraian_arsip, $no_urut_berkas, $no_box, $no_rak, $bentuk_penataan, $nama_pemberkas, $indeks_judul_arsip, $tingkat_pengembangan_arsip, $nilai_guna_arsip, $kondisi_arsip, $status, $pemerian_berkas, $jumlah_berkas, $keterangan, $retensi_aktif, $retensi_inaktif, $tanggal_masuk, $lampirkan_file, $no_surat_arsip);
    } else {
    ...
    if ($stmt->execute()) {
        $_SESSION['flash_message'] = "Data berhasil ditambahkan atau diperbarui";
...
<h2 class="h2">Edit Data Arsip</h2>
<form class="cari" method="POST" action="edit.php" enctype="multipart/form-data">
    <div class="field" tabindex="1">
        <label for="tahun-arsip">
            <i class="fa-solid fa-calendar-days"></i>Tahun Arsip
        </label>
        <input id="tahun-arsip" name="tahun-arsip" style="color: #000" type="text" value="<?php echo htmlspecialchars($arsip_data['tahun_arsip']); ?>" placeholder="Masukkan Tahun Arsip" required>
    </div>
    <div class="field" tabindex="2">
        <label for="skpd">
            <i class="fa-regular fa-building"></i>SKPD
        </label>
        <input id="skpd" name="skpd" style="color: #000" type="text" value="<?php echo htmlspecialchars($arsip_data['skpd']); ?>" placeholder="Masukkan SKPD" required>
    </div>
    <div class="field" tabindex="3">
        <label for="pokok-masalah">
            <i class="fa-solid fa-circle-exclamation"></i>Pokok Masalah
        </label>
        <input id="pokok-masalah" name="pokok-masalah" style="color: #000" type="text" value="<?php echo htmlspecialchars($arsip_data['pokok_masalah']); ?>" placeholder="Masukkan Pokok Masalah" required>
    </div>
<div class="field" tabindex="4">
    <label for="kode-klasifikasi">
        <i class="fa-solid fa-list"></i>Kode Klasifikasi
    </label>
    <input type="text" id="kode-klasifikasi" name="kode-klasifikasi" readonly value="<?php echo htmlspecialchars($arsip_data['kode_klasifikasi']); ?>" required>
    <input type="hidden" id="selectedKode" name="kode-klasifikasi" value="<?php echo htmlspecialchars($arsip_data['kode_klasifikasi']); ?>" required>
...
<div class="field" tabindex="9">
    ...
    <textarea id="uraian-arsip" name="uraian-arsip" style="padding-bottom: 2rem; color: #000;" placeholder="Masukkan Uraian Arsip" required><?php echo htmlspecialchars($arsip_data['uraian_arsip']); ?></textarea>
</div>
<div class="field" tabindex="5">
    <label for="tanggal_masuk">
        <span><i class="bi bi-calendar-date"></i></span>Tanggal Input Berkas
    </label>
    <input id="tanggal-masuk" name="tanggal-masuk" style="color: #000" type="date" value="<?php echo htmlspecialchars($arsip_data['tanggal_masuk']); ?>" placeholder="Masukkan Tanggal Input Berkas" required>
</div>
    <div class="field" tabindex="5">
        <label for="no-urut-berkas">
            <span><i class="bi bi-file-binary"></i></span>No Urut Berkas
        </label>
        <input id="no-urut-berkas" name="no-urut-berkas" style="color: #000" type="text" value="<?php echo htmlspecialchars($arsip_data['no_urut_berkas']); ?>"  placeholder="Masukkan No Urut Berkas" required>
    </div>
    <div class="field" tabindex="6">
        <label for="no-box">
            <i class="fa-solid fa-boxes-stacked"></i>No BOX
        </label>
        <input id="no-box" name="no-box" style="color: #000" type="text" value="<?php echo htmlspecialchars($arsip_data['no_box']); ?>" placeholder="Masukkan No BOX" required>
    </div>
    <div class="field" tabindex="7">
        <label for="no-rak">
            <span><i class="bi bi-bookshelf"></i></span>No RAK
        </label>
        <input id="no-rak" name="no-rak" style="color: #000" type="text" value="<?php echo htmlspecialchars($arsip_data['no_rak']); ?>" placeholder="Masukkan No RAK" required>
    </div>
    <div class="field" tabindex="8">
        <label for="bentuk-penataan">
            <span><i class="bi bi-ui-radios-grid"></i></span>Bentuk Penataan
        </label>
        <input id="bentuk-penataan" name="bentuk-penataan" style="color: #000" type="text" value="<?php echo htmlspecialchars($arsip_data['bentuk_penataan']); ?>" placeholder="Masukkan Bentuk Penataan" required>
    </div>
    <div class="field" tabindex="9">
        <label for="nama-pemberkas">
            <span><i class="bi bi-person"></i></span>Nama Pemberkas
        </label>
        <input id="nama-pemberkas" name="nama-pemberkas" style="color: #000" type="text" value="<?php echo htmlspecialchars($arsip_data['nama_pemberkas']); ?>" placeholder="Masukkan Nama Pemberkas" required>
    </div>
    <div class="field" tabindex="10">
        <label for="no-surat-arsip">
            <span><i class="bi bi-123"></i></span>No Surat Arsip
        </label>
        <input id="no-surat-arsip" name="no-surat-arsip" style="color: #000" type="text" value="<?php echo htmlspecialchars($arsip_data['no_surat_arsip']); ?>" placeholder="Masukkan No Surat Arsip" required>
    </div>
    <div class="field" tabindex="11">
    <label for="indeks-judul-arsip">
        <i class="fa-solid fa-heading"></i>Indeks/Judul Arsip
    </label>
    <textarea id="indeks-judul-arsip" name="indeks-judul-arsip" style="padding-bottom: 2rem; color: #000;" placeholder="Masukkan Indeks/Judul Arsip" required><?php echo htmlspecialchars($arsip_data['indeks_judul_arsip']); ?></textarea>
</div>
<div class="field" tabindex="12">
    ...
    <select id="tingkat-perkembangan-arsip" name="tingkat-perkembangan-arsip" required>
        <option value="" disabled <?php echo empty($arsip_data['tingkat_pengembangan_arsip']) ? 'selected' : ''; ?>>Pilih Tingkat Perkembangan Arsip</option>
        <option value="Asli" <?php echo $arsip_data['tingkat_pengembangan_arsip'] === 'Asli' ? 'selected' : ''; ?>>Asli</option>
        <option value="Copy" <?php echo $arsip_data['tingkat_pengembangan_arsip'] === 'Copy' ? 'selected' : ''; ?>>Copy</option>
        <option value="Tembusan" <?php echo $arsip_data['tingkat_pengembangan_arsip'] === 'Tembusan' ? 'selected' : ''; ?>>Tembusan</option>
    </select>
</div>
<div class="field" tabindex="13">
    ...
    <select id="nilai-guna-arsip" name="nilai-guna-arsip" required>
        <option value="" disabled <?php echo empty($arsip_data['nilai_guna_arsip']) ? 'selected' : ''; ?>>Pilih Nilai Guna Arsip</option>
        <option value="Administrasi" <?php echo $arsip_data['nilai_guna_arsip'] === 'Administrasi' ? 'selected' : ''; ?>>Administrasi</option>
        <option value="Keuangan" <?php echo $arsip_data['nilai_guna_arsip'] === 'Keuangan' ? 'selected' : ''; ?>>Keuangan</option>
        <option value="Hukum" <?php echo $arsip_data['nilai_guna_arsip'] === 'Hukum' ? 'selected' : ''; ?>>Hukum</option>
        <option value="Ilmiah & Teknologi" <?php echo $arsip_data['nilai_guna_arsip'] === 'Ilmiah & Teknologi' ? 'selected' : ''; ?>>Ilmiah & Teknologi</option>
        <option value="Kebuktian/evidental" <?php echo $arsip_data['nilai_guna_arsip'] === 'Kebuktian/evidental' ? 'selected' : ''; ?>>Kebuktian/evidental</option>
        <option value="Informasi" <?php echo $arsip_data['nilai_guna_arsip'] === 'Informasi' ? 'selected' : ''; ?>>Informasi</option>
    </select>
</div>
    <div class="field" tabindex="14">
        ...
        <input id="kondisi-arsip" name="kondisi-arsip" style="color: #000" type="text" value="<?php echo htmlspecialchars($arsip_data['kondisi_arsip']); ?>" placeholder="Masukkan Kondisi Arsip" required>
    </div>
    <div class="field" tabindex="14">
        <label for="retensi-aktif">
            <i class="fa-regular fa-clock"></i>Retensi Aktif
        </label>
        <input id="retensi-aktif" name="retensi-aktif" style="color: #000" type="text" value="<?php echo htmlspecialchars($arsip_data['retensi_aktif']); ?>" placeholder="Masukkan Retensi Aktif (angka saja. Contoh: 3)" required>
    </div>
    <div class="field" tabindex="14">
        <label for="retensi-inaktif">
        <i class="fa-solid fa-clock"></i>Retensi Inaktif
        </label>
        <input id="retensi-inaktif" name="retensi-inaktif" style="color: #000" type="text"  value="<?php echo htmlspecialchars($arsip_data['retensi_inaktif']); ?>" placeholder="Masukkan Retensi Inaktif (angka saja. Contoh: 3)" required>
    </div>
    ...
    <select id="status" name="status" required>
        <option value="" disabled <?php echo empty($arsip_data['status']) ? 'selected' : ''; ?>>Pilih Status Berkas</option>
        <option value="Statis" <?php echo $arsip_data['status'] === 'Statis' ? 'selected' : ''; ?>>Statis</option>
        <option value="Musnah" <?php echo $arsip_data['status'] === 'Musnah' ? 'selected' : ''; ?>>Musnah</option>
    </select>
</div>
<div class="field" tabindex="16">
    ...
    <textarea id="pemerian-berkas" name="pemerian-berkas" style="padding-bottom: 2rem; color: #000;" placeholder="Masukkan Asal Berkas" required><?php echo htmlspecialchars($arsip_data['pemerian_berkas']); ?></textarea>
</div>
    <div class="field" tabindex="17">
        <label for="jumlah-berkas">
            <i class="fa-solid fa-arrow-up-1-9"></i>Jumlah Berkas
        </label>
        <input id="jumlah-berkas" name="jumlah-berkas" style="color: #000" type="text" value="<?php echo htmlspecialchars($arsip_data['jumlah_berkas']); ?>" placeholder="Masukkan Jumlah Berkas" required>
    </div>
    <div class="field" tabindex="18">
    <label for="keterangan">
        <i class="fa-solid fa-t"></i>Keterangan
    </label>
    <textarea id="keterangan" name="keterangan" style="color: #000" placeholder="Masukkan Keterangan" required><?php echo htmlspecialchars($arsip_data['keterangan']); ?></textarea>
</div>
<div class="field" tabindex="19">
    <label for="lampirkan-file">
        <i class="fa-solid fa-file-upload"></i>Lampirkan File
    </label>
    <input id="lampirkan-file" name="lampirkan-file" style="color: #000" type="file" title="Inputkan file berformat PDF atau JPG saja">
    <span id="file-format-info" style="font-size: 14px; color: #fff; margin-left: 10px;">*Inputkan file berformat PDF/JPG saja</span>
    <?php if (!empty($arsip_data['lampirkan_file'])): ?>
        <p style="font-size: 14px; color: #fff; margin-left: 10px;">
            File yang telah diunggah: 
            <a href="/Projek-SiADI/uploads/<?php echo htmlspecialchars($arsip_data['lampirkan_file']); ?>" target="_blank">
                <?php echo htmlspecialchars($arsip_data['lampirkan_file']); ?>
            </a>
        </p>
    <?php endif; ?>
</div>
    ...
    <button style="background-color: 	#00FF00; color: #000" type="submit"><i class="fa-regular fa-pen-to-square"></i>Update Data</button>
